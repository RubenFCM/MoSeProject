                ...
                Column(modifier = Modifier.fillMaxSize()) {
                    AsyncImage(
                        model = ImageRequest.Builder(LocalContext.current)
                            .data(Constants.BASE_URL_IMAGES_W220 + data[res].results[0].poster_path)
                            .crossfade(true)
                            .scale(Scale.FIT)
                            .build(),
                        ...
                        modifier = Modifier
                            ...
                            .clickable {
                                navController.navigate(
                                    route = AppScreen.FilmID.route + "/" + data[res].results[0].id
                                )
                            }
                    )
                }
                ...
                Column(modifier = Modifier.fillMaxSize()) {
                    AsyncImage(
                        ...
                        modifier = Modifier
                            ...
                            .clickable {
                                navController.navigate(
                                    route = AppScreen.SerieID.route + "/" + data[res].results[0].id
                                )
                            }
                    )
                }
